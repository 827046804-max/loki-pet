...
### 桌面启动程序
- 创建带像素风图标的 `LokiPet.app`
- 图标：像素风格的 Loki 猫形象
- 位置：`~/Desktop/LokiPet.app`
- 双击即可启动
...
### 窗口交互优化
- 窗口置顶设置（`WindowStaysOnTopHint`）
- 左键拖拽移动桌宠位置
- 右键菜单切换动作
- 添加 `WA_ShowWithoutActivating` 防止抢占焦点

### 桌面启动程序
创建 `loki-pet.command` 桌面快捷方式，双击即可启动
